...
public final class VusionIntegrationManager: VusionTrackingDelegate {
    ...
    public func startAfterMapLoad() {
        ...
        // Post initialization starting status
        let initializingStatus = VusionStatusUpdate(status: .initializing)
        NotificationCenter.default.post(
            name: NSNotification.Name("VusionStatusUpdateAvailable"),
            object: nil,
            userInfo: ["status": initializingStatus]
        )
        ...
        do {
            ...
            // Post initialization success status
            let initializedStatus = VusionStatusUpdate(status: .initialized)
            NotificationCenter.default.post(
                name: NSNotification.Name("VusionStatusUpdateAvailable"),
                object: nil,
                userInfo: ["status": initializedStatus]
            )
        } catch {
            ...
            // Also forward to RetailBrainManager delegate
            RetailBrainManager.shared.delegate?.vusionDidFailToInitialize(error)
    ...
    private func subscribeToLocationUpdates() {
        ...
        .sink { [weak self] aisleName, modularName, deviceId, rssi in
            ...
            print("[RetailBrain][Vusion] Aisle: \(aisleName) | Modular: \(modularName) | Device: \(deviceId) | RSSI: \(rssi)")

            self?.delegate?.vusionDidUpdateLocation(update)
            
            // Also forward to RetailBrainManager delegate
            RetailBrainManager.shared.delegate?.vusionDidUpdateLocation(update)
            
            // Post notification to trigger toast display
            NotificationCenter.default.post(
                name: NSNotification.Name("VusionLocationUpdateAvailable"),
                object: nil,
    ...
    public func vusionDidUpdateLocation(_ update: VusionLocationUpdate) {
        // Forward to RetailBrainManager delegate
        RetailBrainManager.shared.delegate?.vusionDidUpdateLocation(update)
    }
